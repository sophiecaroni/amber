...
from amber.features.vision_extraction import extract_vision_features
...
def extract_vision_table(test: bool, save: bool = False):
    vision_df = extract_vision_features()
    if save:
        out_dir = io.get_tables_path()
        fname = 'vision_features.csv'
        vision_df.to_csv(out_dir / fname)
        print(f'\n\tExporting {fname}...\n\tSaved ✅')
    else:
        print(
            f"\nFinal df: \n\t{vision_df}"
