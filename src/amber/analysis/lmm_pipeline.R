...
df <- raw_df %>%
  select(sid, eye_cond, interv, interv_eff, age, all_of(metric)) %>%  # select columns of interest for this analysis
...
# Full model: all interactions up to 3-way among primary factors + all main effects + age covariate
full_formula <- y ~ eye_cond * interv * interv_eff +  # * expands to all interactions up to 3-way, equivalent to (sum of all factors)^3
                    age +
...
no_3way_formula <- y ~ (eye_cond + interv + interv_eff)^2 +
                        age +
                        (1 + interv_eff | sid) +
                        (1 + eye_cond | sid)

# Main effects only
main_formula <- y ~ eye_cond + interv + interv_eff + age +
                    (1 + interv_eff | sid) +
                    (1 + eye_cond | sid)
...
# Run LRT to verify if removing the 3-way interaction is significant
cat("\n--- LRT: full vs no-3-way ---\n")
full_vs_no3 <- anova(m_full, m_no_3way)
print(full_vs_no3)
p_full_vs_no3 <- tryCatch(full_vs_no3$`Pr(>Chisq)`[2], error = function(e) NA_real_)  # get p-values
...
# Step down if m_full is not valid or if LRT didn't detect a difference to no-3-way
if (!valid(m_full) || is.na(p_full_vs_no3) || p_full_vs_no3 >= 0.05) {
  best_model <- "m_no_3way"
}
# If no-3-way was previously selected, step down if m_no_3way is not valid or if LRT didn't detect a difference to main
