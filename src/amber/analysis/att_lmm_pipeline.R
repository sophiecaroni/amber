...
df <- raw_df %>%
    ...
    mutate(
        ...
        age=as.numeric(age),
        att_val=as.numeric(att_val),
    )

# Compute the percentage of change of attention features from BL to ST and from BL to FU
bl_df <- df %>%
    filter(interv_eff == "BL") %>%
    select(sid, amb_type, eye_cond, interv, att_type, att_val_bl=att_val)  # rename att_val col into att_val_bl

# Join bl_df in df and use att_val_bl to compute the percentage of change
pct_df <- df %>%
    filter(interv_eff %in% c("ST", "FU")) %>%
    left_join(bl_df, by=c("sid", "amb_type", "eye_cond", "interv", "att_type")) %>%
    mutate(
        perc_change=(att_val - att_val_bl) / att_val_bl * 100,  # create new column with perc_change
    ...
    select(-att_val_bl, -interv_eff) %>%  # remove cols anymore needed
